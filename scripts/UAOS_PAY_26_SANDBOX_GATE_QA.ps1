...
&{
 ...
 function F($m){Write-Host "[PAY-26 QA FAIL] $m" -ForegroundColor Red;throw $m}
 $Required=@("$A\public\paypal\sandbox-approval-gate.json","$A\public\paypal\sandbox-approval-gate.html","$A\public\paypal\paypal-sandbox-config.placeholder.json","$A\public\commercial\payment-activation-checklist.html","$A\public\qa\payment-admin-status.html")
 ...
 @{result="PASS";build="PASS";sandbox="LOCKED_PENDING_APPROVAL";livePayment="FORBIDDEN";timestamp=$T}|ConvertTo-Json -Depth 8|Set-Content "$D\UAOS_PAY_26_SANDBOX_GATE_QA_$T.json" -Encoding UTF8
 Write-Host "UAOS PAY-26 SANDBOX GATE QA PASS" -ForegroundColor Green
}
