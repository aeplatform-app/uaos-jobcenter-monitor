...
&{
 ...
 function F($m){Write-Host "[PAY-65 QA FAIL] $m" -ForegroundColor Red;throw $m}
 $Required=@("$A\public\commercial\payment-freeze.json","$A\public\commercial\public-launch-checklist.html","$A\public\qa\payment-freeze-dashboard.html","$A\public\commercial\commercial-launch-guard.json","$A\public\commercial\final-payment-lock.json","$A\public\commercial\buyer-notice.html")
 ...
 @{result="PASS";build="PASS";paymentFreeze="ACTIVE";paymentActivation="NOT_ACTIVE";timestamp=$T}|ConvertTo-Json -Depth 8|Set-Content "$D\UAOS_PAY_65_PAYMENT_FREEZE_ROUTE_QA_$T.json" -Encoding UTF8
 Write-Host "UAOS PAY-65 PAYMENT FREEZE ROUTE QA PASS" -ForegroundColor Green
}
