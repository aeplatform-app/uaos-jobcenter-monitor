...
$r=Invoke-WebRequest "http://127.0.0.1:5199/api/release/report" -UseBasicParsing
"UAOS V9 PASS $(Get-Date)`nBACKEND $($b.StatusCode)`nFRONTEND $($f.StatusCode)`nRELEASE REPORT $($r.StatusCode)" | Set-Content "agent-output\UAOS_V9_STATUS.txt"
git add .
git commit -m "Add UAOS V9 production hardening launch layer"
Start-Process "http://127.0.0.1:5180"
notepad "agent-output\UAOS_V9_STATUS.txt"
